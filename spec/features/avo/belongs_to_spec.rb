...
RSpec.feature "belongs_to", type: :feature do
  ...
  context "show" do
    ...
    describe "with user attached" do
      ...
      it { is_expected.to have_link admin.name, href: "/admin/resources/users/#{admin.slug}?via_record_id=#{post.id}&via_resource_class=Avo%3A%3AResources%3A%3APost" }
    end
  ...
  context "edit" do
    ...
    describe "without user attached" do
      ...
      it "changes the user" do
        ...
        expect(page).to have_link admin.name, href: "/admin/resources/users/#{admin.slug}?via_record_id=#{post.id}&via_resource_class=Avo%3A%3AResources%3A%3APost"
      end
    end
    ...
    describe "with user attached" do
      ...
      it "changes the user" do
        ...
        expect(current_path).to eql "/admin/resources/posts/#{post.id}"
        expect(page).to have_link second_user.name, href: "/admin/resources/users/#{second_user.slug}?via_record_id=#{post.id}&via_resource_class=Avo%3A%3AResources%3A%3APost"
      end
