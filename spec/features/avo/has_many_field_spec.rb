...
RSpec.feature "HasManyField", type: :feature do
  ...
  context "show" do
    ...
    let(:url) { "/admin/resources/users/#{user.to_param}/posts?turbo_frame=has_many_field_posts&view_type=table" }

    describe "without a related post" do
      it { is_expected.to have_text "No record found" }
      ...
      it "creates a post" do
        ...
        expect(page).to have_current_path "/admin/resources/posts/new?via_record_id=#{user.to_param}&via_relation=user&via_relation_class=User"
    ...
    describe "with a related post" do
      ...
      it "displays valid links to resources" do
        ...
        # grid view button
        expect(page).to have_selector "[data-control='view-type-toggle-grid'][href='/admin/resources/users/#{user.to_param}/posts?turbo_frame=has_many_field_posts&view_type=grid']"

        # create new button
        expect(page).to have_link("Create new post", href: "/admin/resources/posts/new?via_record_id=#{user.to_param}&via_relation=user&via_relation_class=User")

        # attach button
        expect(page).to have_link("Attach post", href: /\/admin\/resources\/users\/#{user.to_param}\/posts\/new/)

        ## Table Rows
        # show link
        show_path = "/admin/resources/posts/#{post.id}?via_record_id=#{user.to_param}&via_resource_class=Avo%3A%3AResources%3A%3AUser"
        expect(page).to have_css("a[data-control='show'][href='#{show_path}']")

        # id field show link
        expect(field_element_by_resource_id("id", post.id)).to have_css("a[href='/admin/resources/posts/#{post.id}?via_record_id=#{user.to_param}&via_resource_class=Avo%3A%3AResources%3A%3AUser']")

        # edit link
        edit_path = "/admin/resources/posts/#{post.id}/edit?via_record_id=#{user.to_param}&via_resource_class=Avo%3A%3AResources%3A%3AUser"
        expect(page).to have_selector("[data-component='resources-index'] a[data-control='edit'][data-resource-id='#{post.id}'][href='#{edit_path}']")

        # detach form
        form = "form[action='/admin/resources/users/#{user.to_param}/posts/#{post.id}']"
        ...
        # destroy form
        form = "form[action='/admin/resources/posts/#{post.id}']"
        expect(page).to have_selector("[data-component='resources-index'] #{form}")
  ...
  describe "scope" do
    ...
    subject do
      visit "/admin/resources/users/#{user.to_param}/comments?turbo_frame=has_many_field_show_comments"
      page
    end
