      ...
      context "without an association" do
        context "when not filling the poly association" do
          it "creates the comment" do
            ...
            expect(find_field_value_element("user")).to have_link user.name, href: "/admin/resources/users/#{user.slug}?via_record_id=#{Comment.last.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
        ...
        context "when filling the poly association" do
          describe "creating a polymorphic association" do
            it "creates the comment" do
              ...
              expect(find_field_value_element("body")).to have_text "Sample comment"
              expect(page).to have_link post.name, href: "/admin/resources/posts/#{post.id}?via_record_id=#{Comment.last.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
    ...
    describe "within a parent model" do
      ...
      context "in a project show page" do
        it "has the comment listed" do
          ...
          expect(find_field_value_element("user")).to have_link user.name, href: "/admin/resources/users/#{user.slug}?via_record_id=#{comment.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
  ...
  describe "searchable" do
    ...
    describe "with associated record" do
      it "prefills the associated record details" do
        visit "/admin/resources/reviews/new?via_relation=reviewable&via_relation_class=Team&via_record_id=#{team.id}"
