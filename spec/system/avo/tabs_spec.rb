  ...
  describe "tabs" do
    context "show" do
      it "switches the tabs" do
        ...
        expect(find('turbo-frame[id="has_many_field_show_posts"]')).to have_link "Create new post", href: "/admin/resources/posts/new?via_record_id=#{user.to_param}&via_relation=user&via_relation_class=User"
        ...
        expect(find("turbo-frame#has_and_belongs_to_many_field_show_teams")).to have_link "Create new team", href: "/admin/resources/teams/new?via_record_id=#{user.to_param}&via_relation=users&via_relation_class=User"
      end
