...
RSpec.describe "TestHelpers", type: :system do
  ...
  describe "field_wrapper on self resource" do
    context "index" do
      ...
      it "finds the wrapper on grid" do
        ...
        users.each do |user|
          grid_item_wrapper = grid_field_wrapper(record_id: user.id)
      ...
      it "finds the wrapper" do
        ...
        users.each do |user|
          first_name_wrapper = within(has_and_belongs_to_many_users) {
            index_field_wrapper(id: "first_name", type: "text", record_id: user.id)
          }

          first_name_wrapper_without_type = within(has_and_belongs_to_many_users) {
            index_field_wrapper(id: "first_name", record_id: user.id)
          }
  ...
  describe "isolated tests" do
    ...
    context "run_action" do
      ...
      it "runs the row action" do
        visit "admin/resources/fish"

        open_row_action(record_id: fish.id, action_name: "Release fish")
    ...
    context "menu" do
      it "expand and collapse menu" do
        visit "admin/resources/projects"
        expect(page).to have_link "Projects", href: "/admin/resources/projects"
        expect(page).to have_link "Teams", href: "/admin/resources/teams"
        expect(page).to have_link "Reviews", href: "/admin/resources/reviews"
        toggle_collapsable("company")
        expect(page).to_not have_link "Projects", href: "/admin/resources/projects"
        expect(page).to_not have_link "Teams", href: "/admin/resources/teams"
        expect(page).to_not have_link "Reviews", href: "/admin/resources/reviews"
        toggle_collapsable("company")
        expect(page).to have_link "Projects", href: "/admin/resources/projects"
        expect(page).to have_link "Teams", href: "/admin/resources/teams"
        expect(page).to have_link "Reviews", href: "/admin/resources/reviews"
